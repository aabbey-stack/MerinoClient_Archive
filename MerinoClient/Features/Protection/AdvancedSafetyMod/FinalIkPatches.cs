...
internal static class FinalIkPatches
{
    public static void ApplyPatches(HarmonyLib.Harmony harmony)
    {
        harmony.Patch(
            typeof(IKSolverHeuristic).GetMethods()
                .First(m => m.Name.Equals("IsValid") && m.GetParameters().Length == 1),
    ...
    {
        if (__instance is not { maxIterations: > 64 }) return true;
